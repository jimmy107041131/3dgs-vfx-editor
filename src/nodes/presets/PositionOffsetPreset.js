...
function buildPositionOffsetGraph(graph) {
  ...
  // ── Split offset ──
  const splitO = LiteGraph.createNode('3dgs/utility/SplitVec3');
  splitO.pos = [300, 50];
  graph.add(splitO);
  inOffset.connect(0, splitO, 0);

  // ── Add components ──
  const addX = LiteGraph.createNode('3dgs/math/Math');
  graph.add(addX);
  addX.pos = [520, 50];
  splitC.connect(0, addX, 0);
  splitO.connect(0, addX, 1);

  const addY = LiteGraph.createNode('3dgs/math/Math');
  graph.add(addY);
  addY.pos = [520, 200];
  splitC.connect(1, addY, 0);
  splitO.connect(1, addY, 1);

  const addZ = LiteGraph.createNode('3dgs/math/Math');
  graph.add(addZ);
  addZ.pos = [520, 350];
