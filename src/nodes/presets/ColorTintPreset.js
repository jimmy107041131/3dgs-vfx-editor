...
function buildColorTintGraph(graph) {
  ...
  // ── Split rgba → r,g,b,w ──
  const splitRgba = LiteGraph.createNode('3dgs/GPU/utility/BreakVec4 (GPU)');
  splitRgba.pos = [300, 200];
  graph.add(splitRgba);
  inRgba.connect(0, splitRgba, 0);

  // ── Split tint → tr,tg,tb ──
  const splitTint = LiteGraph.createNode('3dgs/GPU/utility/BreakVec3 (GPU)');
  splitTint.pos = [300, 50];
  graph.add(splitTint);
  inTint.connect(0, splitTint, 0);

  // ── Multiply: channel × tint ──
  const mulR = LiteGraph.createNode('3dgs/GPU/math/Math (GPU)');
  graph.add(mulR);
  mulR.pos = [520, 60];
  ...
  const mulG = LiteGraph.createNode('3dgs/GPU/math/Math (GPU)');
  graph.add(mulG);
  mulG.pos = [520, 220];
  ...
  const mulB = LiteGraph.createNode('3dgs/GPU/math/Math (GPU)');
  graph.add(mulB);
  mulB.pos = [520, 380];
  ...
  const make = LiteGraph.createNode('3dgs/GPU/utility/MakeVec4 (GPU)');
  make.pos = [740, 200];
  graph.add(make);
